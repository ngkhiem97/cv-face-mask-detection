...
def main():
    ...
    while True:
        ...
        for face in faces:
            ...
            xmin = max(xmin - box_width * 0.2, 0)
            ymin = max(ymin - box_height * 0.2, 0)
            xmax = min(xmax + box_width * 0.2, width)
            ymax = min(ymax + box_height * 0.2, height)
            image_data = Image.fromarray(image[:,:,::-1])
            image_crop = image_data.crop((int(xmin), int(ymin), int(xmax), int(ymax)))
            image_crop.save("face_crop.jpg")
